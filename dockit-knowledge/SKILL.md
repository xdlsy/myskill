---
name: dockit-knowledge
description: 为代码仓构建知识库（Knowledge Base），使用 Mermaid + C4 模型 + ADR 承载系统架构图、模块内部架构、跨模块流程图和架构决策记录。支持独立使用，也可作为 dockit-init 阶段 3 的增强步骤。输出到 docs/knowledge/ 目录。
---
...
- **管道模式**（可选）：读取 `.dockit/phase0/repo-profile.md` 和已有 `AGENTS.md` 自动推断模块结构
...
**管道模式**——从已有产物中推断：
- 读取 `.dockit/phase0/repo-profile.md` 获取模块结构、语言/框架信息
...
为步骤 0 收集到的每个模块，**派发独立子 Agent** 并调用 `dockit-module` skill 生成 `docs/knowledge/modules/<module-name>.md`。
...
**子 Agent 调用示例**：
```
Agent prompt: "使用 dockit-module skill 为模块 [{模块名}] 生成知识文档。
模块信息如下：
- 名称：{模块名}
- 技术栈：{技术栈}
- 职责：{一句话描述}
...
子 Agent 会自动：
1. 调用 dockit-module skill 执行发现→生成→更新索引→确认流程
...
> 
> 模块蓝图的具体结构和模板由 `dockit-module` skill 定义，详见该 skill 的步骤 1。

### 步骤 4：流程蓝图（Runtime View）—— 派发子 Agent

为步骤 0 收集到的每条核心流程，**派发独立子 Agent** 并调用 `dockit-flow` skill 生成 `docs/knowledge/flows/<flow-name>.md`。
...
**子 Agent 调用示例**：
```
Agent prompt: "使用 dockit-flow skill 为流程 [{流程名}] 生成知识文档。
流程信息如下：
- 名称：{流程名}
...
子 Agent 会自动：
1. 调用 dockit-flow skill 执行发现→生成→更新索引→确认流程
...
>
> 流程蓝图的具体结构和模板由 `dockit-flow` skill 定义，详见该 skill 的步骤 1。
...
为步骤 0 收集到的每个关键决策，**派发独立子 Agent** 并调用 `dockit-adr` skill 生成 `docs/knowledge/decisions/adr-NNNN-<slug>.md`。
...
**子 Agent 调用示例**：
```
Agent prompt: "使用 dockit-adr skill 为决策 [{决策标题}] 创建 ADR 文档。
决策信息如下：
- 决策标题：{标题}
- 背景上下文：{背景}
- 决策内容：{决策}
...
子 Agent 会自动：
1. 调用 dockit-adr skill 执行输入校验→生成 ADR→写入文件流程
...
> ADR 的格式（MADR + 编码要点 POS/NEG/ALT/IMP/REF）、命名规范、输入校验由 `dockit-adr` skill 定义，详见该 skill。
...
4. 写入完成报告到 `.dockit/knowledge/report.md`：
...
- 所有文档间的交叉引用使用相对路径链接
- 模块蓝图、流程蓝图、ADR 的具体约束详见各自子 skill（`dockit-module`、`dockit-flow`、`dockit-adr`）
