---
name: dockit-skills
description: 扫描代码仓中可封装为 skill 的自动化能力，生成领域能力目录（docs/skills/）。依赖阶段 0-3 产出的 AGENTS.md 和 ARCHITECTURE.md。由 dockit-init 阶段 4 触发，也可独立使用。
---
...
2. 确认重点：
   - "检测到的 skill 候选是否遗漏？"
3. 用户确认后，将完成报告写入 `.dockit/phase4a/report.md`：
