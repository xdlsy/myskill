...
async function build() {
  ...
  // ──────────────── S1: Title ────────────────
  {
    ...
    s.addText("dockit", { x: 1.2, y: 1.2, w: 7.6, h: 1.2, fontSize: 64, fontFace: "Arial Black", color: C.white, bold: true, margin: 0 });
  ...
  // ──────────────── S2: 什么是 dockit ────────────────
  {
    ...
    s.addText("什么是 dockit？", { x: 0.8, y: 0.35, w: 8.4, h: 0.65, fontSize: 32, fontFace: "Georgia", color: C.textDark, bold: true, margin: 0 });
  ...
  // ──────────────── S5: 工作流全景 ────────────────
  {
    ...
    s.addText("Phase 5: dockit-claude", { x:0.7, y:ey, w:3.1, h:0.32, fontSize:14, fontFace:"Calibri", color:C.white, bold:true, margin:0 });
    ...
    // Support skills
    [{ t:"dockit-adr", d:"架构决策记录（MADR 格式）" },{ t:"dockit-learning", d:"经验捕获与技能升级（L3 持续积累）" }].forEach((sk,i)=>{
    ...
    s.addText("dockit-init — 主流程编排器，一键串联 Phase 0–5", { x:2.5, y:4.6, w:5, h:0.55, fontSize:14, fontFace:"Calibri", color:C.white, align:"center", valign:"middle", margin:0 });
  }

  // ──────────────── S6-S10: Phase Details ────────────────
  const detailSlides = [
    { t:"Phase 0: dockit-repo-profile", st:"代码仓探索与画像采集 — L0 层数据基础", i:"search", c:C.primary,
      ...
      out:"→ .dockit/phase0/repo-profile.md" },
    { t:"Phase 1: dockit-root-agents", st:"根目录 AGENTS.md 生成 — L0 热记忆层（≤150 行）", i:"file", c:C.teal,
    ...
    { t:"Phase 2: dockit-module-agents", st:"子模块 AGENTS.md 批量生成 — L0 模块热记忆层", i:"cubes", c:C.purple,
    ...
    { t:"Phase 3: dockit-architecture", st:"架构文档 — matklad 三段式范式（Bird's-eye → Code map → Cross-cutting）", i:"sitemap", c:C.primary,
    ...
    { t:"Phase 4: dockit-knowledge-init", st:"知识架构骨架 — 对应五层架构的 L1+L2+L3 层", i:"book", c:C.teal,
  ...
  {
    const s = pres.addSlide();
    const icp = whiteHeader(s, "dockit-adr", "架构决策记录 — MADR 格式 · L2 决策知识层 · AI 可程序化检索", C.amber, "");
  ...
  {
    const s = pres.addSlide();
    const icp = whiteHeader(s, "dockit-learning", "持续学习与经验管理 — L3 经验库层 · 自动化知识闭环", C.green, "");
  ...
  // ──────────────── S14: Phase 5: dockit-claude ────────────────
  {
    const s = pres.addSlide();
    const icp = whiteHeader(s, "Phase 5: dockit-claude", "Claude Code 原生适配 — 接通 Anthropic 四层加载模型", C.primary, "");
  ...
  // ──────────────── S15: dockit-init 主流程 ────────────────
  {
    ...
    s.addText("dockit-init — 主流程编排器", { x:0.8, y:0.35, w:8.4, h:0.65, fontSize:30, fontFace:"Georgia", color:C.textDark, bold:true, margin:0 });
  ...
  {
    const s = pres.addSlide();
    darkHeader(s, "开始使用", "一句话启动 dockit 文档化流程");
    ...
    s.addText("→ 触发 dockit-init，自动执行 Phase 0-5 全流程，构建 L0-L4 知识架构", { x:1.2, y:2.85, w:7.6, h:0.3, fontSize:11, fontFace:"Calibri", color:C.textMuted, align:"center", margin:0 });

    [{ cmd:"探索项目结构", sk:"dockit-repo-profile", d:"快速了解陌生代码仓全貌" },
     { cmd:"创建 ADR", sk:"dockit-adr", d:"记录关键架构决策及理由" },
     { cmd:"记录学习经验", sk:"dockit-learning", d:"捕获错误/纠正/最佳实践" }]
    .forEach((uc,i)=>{
  ...
  // ════════════════════════════════════════════════════════════
  const outPath = "/Users/lsy/skills/pptx/dockit-introduction.pptx";
