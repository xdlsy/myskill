---
name: dockit-adr
description: 创建单篇架构决策记录（ADR）文档，使用 MADR 格式 + 编码要点（POS/NEG/ALT/IMP/REF）。输出到 docs/knowledge/decisions/ 目录，与 dockit-knowledge 的 ADR 索引和蓝图统一管理。
---
...
- **`dockit-knowledge`**：负责蓝图全貌（C4 图 + 流程图 + ADR 索引），批量生成时调用本技能创建单篇 ADR
- **本技能**：专注于单篇 ADR 的创建和编辑，可独立使用，也可被 `dockit-knowledge` 调用
...
ADR 保存至 `docs/knowledge/decisions/` 目录（与 `dockit-knowledge` 的输出目录一致）。
