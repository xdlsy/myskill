---
name: dockit-claude
description: 用于将 dockit 文档体系适配为 Claude Code 原生加载结构。触发场景：(1) dockit-init 完成后的收尾阶段，(2) 已有 AGENTS.md 需要接通 Claude Code 加载链，(3) 用户要求"适配 Claude Code"、"配置 Claude 规则"、(4) 需要将 dockit-learning 的 activator.sh 注册为 Hook。
---

# Dockit 适配 Claude Code

## 概览

将 dockit 文档体系适配为 Claude Code 原生加载结构，做四件事：
...
4. 配置 `dockit-learning` 的 `activator.sh` 为 `UserPromptSubmit` Hook

**前置条件：** 项目根目录已有 `AGENTS.md`（由 `dockit-init` 生成或手动编写）。
...
`docs/skills/` 存放项目级可复用能力（由 `dockit-skills` 生成）。Claude Code 原生从 `.claude/skills/` 加载 skills，因此创建软链接使其可被发现：
...
### 步骤 4：配置 dockit-learning Hook

让 `dockit-learning` 的 `activator.sh` 在每次用户输入后自动触发，提醒 Claude 在任务完成后评估是否有可提取的知识。

#### 3.1 部署 activator.sh 到项目本地

```bash
mkdir -p .claude/scripts/dockit-learning
cp ~/.claude/skills/dockit-learning/scripts/activator.sh .claude/scripts/dockit-learning/
chmod +x .claude/scripts/dockit-learning/activator.sh
```
    ...
    "UserPromptSubmit": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": ".claude/scripts/dockit-learning/activator.sh"
          }
        ]
      }
    ]
...
```bash
# 列出所有源码模块 AGENTS.md（排除 docs/ 和 .dockit/）
find . -name "AGENTS.md" -not -path "./.git/*" -not -path "./.dockit/*" -not -path "./docs/*" | sort
...
创建 `mkdir -p .dockit/phase5 &&` 写入 `.dockit/phase5/report.md`：
...
- `.claude/scripts/dockit-learning/`：已部署 `activator.sh`
...
| `.claude/scripts/dockit-learning/activator.sh` | Hook 脚本 | ✓ |
...
**注意：** 展示树形一览给用户确认后，再写入报告文件。若 `.dockit/phase5/report.md` 已存在，展示 diff 让用户选择保留/合并/替换。
