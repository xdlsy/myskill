---
name: aidoc-adapt-opencode
description: 将 aidoc 文档体系适配为 OpenCode 原生加载结构。触发场景：(1) aidoc-build 完成后的收尾阶段，(2) 已有 AGENTS.md 需要接通 OpenCode 配置，(3) 用户要求"适配 OpenCode"、"配置 OpenCode"，(4) 需要将 aidoc-learning 注册为 OpenCode 插件。
---

# Aidoc 适配 OpenCode

将 aidoc 文档体系适配为 OpenCode 原生加载结构，做两件事：

1. 验证 `AGENTS.md` 入口文件（OpenCode 原生读取，无需桥接）
2. 配置 `aidoc-learning` 为 OpenCode TypeScript 插件（`.opencode/plugins/`）

**前置条件：** 项目根目录已有 `AGENTS.md`（由 `aidoc-build` 生成或手动编写），且已运行 `aidoc-adapt-claude`（创建 `.claude/skills/` 软链接，OpenCode 通过 `~/.claude/skills/` 路径发现 skills）。
...
文件：`.opencode/plugins/aidoc-learning.ts`

```typescript
export const AidocLearningPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
...
# 3. 插件文件存在
test -f .opencode/plugins/aidoc-learning.ts && echo "OK" || echo "MISSING"
```

验证要点：
- `AGENTS.md` 存在于项目根目录
- `.claude/skills/` 软链接存在（由 `aidoc-adapt-claude` 创建）
- 所有 `docs/skills/*/SKILL.md` 都有合规的 YAML frontmatter
- `.opencode/plugins/aidoc-learning.ts` 存在
- 与 `.claude/` 目录下 Claude Code 配置无冲突（两套可共存）

### 步骤 4：写入完成报告

创建 `mkdir -p .aidoc/phase5 &&` 写入 `.aidoc/phase5/opencode-report.md`（与 Claude 版的 `report.md` 分开，避免覆盖）：
...
- Skills 发现路径：`~/.claude/skills/`（由 `aidoc-adapt-claude` 配置，{N} 个 skills 可发现）
- Skills frontmatter 合规：{M}/{T}（{P} 已修复）
- `.opencode/plugins/aidoc-learning.ts`：[已创建 / 已存在]
- 生成时间：{时间戳}
...
|------|------|------|
| `AGENTS.md` | 入口文件（原生读取） | ✓ |
| `.claude/skills/` | Skills 软链接（由 aidoc-adapt-claude 创建） | ✓ |
| `.opencode/plugins/aidoc-learning.ts` | Learning 插件 | [已创建/已存在] |
...
**注意：** 展示树形一览给用户确认后，再写入报告文件。若 `.aidoc/phase5/opencode-report.md` 已存在，展示 diff 让用户选择保留/合并/替换。
...
| `.opencode/plugins/aidoc-learning.ts` | 内容相同跳过；不同展示 diff |
...
| 文件 | 最大行数 | 内容 |
|------|---------|------|
| `.opencode/plugins/aidoc-learning.ts` | ~20 | TypeScript 插件 |

## 设计原则

- **AGENTS.md 是唯一真相源。** OpenCode 原生读取 AGENTS.md，无需额外配置文件桥接。
- **Skills 复用 Claude Code 配置。** OpenCode 通过 `~/.claude/skills/` 路径发现 skills，与 `aidoc-adapt-claude` 共享同一套软链接，无需重复配置。
